	...
	$(".jian").click(function() {
		...
		if(quantity == 1) {
			if(confirm("您是否真要删除该条目？")) {
				location = "/goods_ssh/cart_batchDelete.action?cartItemIds=" + id;
			}
		} else {
...
function sendUpdateQuantity(id, quantity) {
	$.ajax({
		async:false,
		cache:false,
		url:"/goods_ssh/cart_updateQuantity.action",
...
function batchDelete() {
	...
	location = "/goods_ssh/cart_batchDelete.action?cartItemIds=" + cartItemIdArray;
}
...
<c:forEach items="${cartItemList }" var="cartItem">
	<tr align="center">
		...
		<td align="left" width="70px">
			<a class="linkImage" href="<c:url value='/book_findByBname.action?bname=${cartItem.book.bname }'/>"><img border="0" width="54" align="top" src="<c:url value='/${cartItem.book.image_b }'/>"/></a>
		</td>
		<td align="left" width="400px">
		    <a href="<c:url value='/book_findByBname.action?bname=${cartItem.book.bname }'/>"><span>${cartItem.book.bname }</span></a>
		</td>
		...
		<td>
			<a href="<c:url value='/cart_batchDelete.action?cartItemIds=${cartItem.cartItemId }'/>">删除</a>
		</td>
	</tr>
</c:forEach>
	...
	<form id="jieSuanForm" action="<c:url value='/cart_loadCartItems.action?'/>" method="post">
